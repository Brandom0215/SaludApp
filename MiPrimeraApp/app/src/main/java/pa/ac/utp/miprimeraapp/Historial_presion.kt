...
class Historial_presion : AppCompatActivity() {
    ...
    private fun cargarHistorial() {
        val listaRegistrosDB = dbHelper.obtenerHistorialPresion(currentUserId)

        val lvHistorial = findViewById<ListView>(R.id.lvHistorial)
        val adapter = PresionAdapter(this, listaRegistrosDB) { position ->
            val registroId = listaRegistrosDB[position].id
            android.app.AlertDialog.Builder(this)
                .setTitle("Eliminar registro")
                .setMessage("¿Deseas eliminar este registro de presión?")
                .setPositiveButton("Sí") { _, _ ->
                    dbHelper.eliminarPresion(registroId)
                    cargarHistorial()
                }
